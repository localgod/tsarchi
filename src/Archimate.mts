...
export class Archimate {
  ...
  public upsertElement(element: Partial<Element> & Pick<Element, "name" | "type">): void {
    ...
    if (existingIndex >= 0) {
      ...
      console.log(
        `Updated element "${existingElement.name}" (Type: ${existingElement.type}) in folder "${folderType.get(folderKey)}".`,
      );
    } else {
      ...
      console.log(
        `Added element "${element.name}" (ID: ${element.id}, Type: ${element.type}) to folder "${folderType.get(folderKey)}".`,
      );
    }
  }
  ...
  public addConnection(
    ...
    return this.viewManager.addConnection(
      viewId,
      sourceObjectId,
      targetObjectId,
      relationshipId,
