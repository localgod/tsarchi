...
export class ViewManager {
  ...
  private elementToView(element: Element): View {
    return {
      ...
      children: Array.isArray(element.child)
        ? (element.child as ViewChild[])
        : element.child
          ? [element.child as ViewChild]
          : [],
  ...
  private addRelationshipConnections(
    viewId: string,
    elementIds: string[],
    diagramObjects: ViewDiagramObject[],
  ): void {
    ...
    relationships.forEach((relationship) => {
      const sourceObject = diagramObjects.find(
        (obj) => obj.archimateElement === relationship.source,
      );
      const targetObject = diagramObjects.find(
        (obj) => obj.archimateElement === relationship.target,
      );
